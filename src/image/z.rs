...
pub struct Args {
    ...
    pub model: Model,
    /// Override path to the model weights directory.
...
pub fn run(args: Args) -> Result<()> {
    ...
    let device = super::utils::device(args.cpu).map_err(|e| anyhow::anyhow!(e))?;
    ...
    let model_dir = match args.model_path {
        Some(ref p) => std::path::PathBuf::from(p),
        None => {
            let dir = crate::models::model_repo_dir(args.model.repo());
            if !dir.exists() {
                anyhow::bail!(
                    "Model not found at {}.\n\nRun `akio pull {}` first.",
                    dir.display(),
    ...
    println!("\nLoading models from: {}", model_dir.display());
    ...
    let text_encoder_config_path = model_dir.join("text_encoder").join("config.json");
    ...
    super::utils::save_image(&image, &args.output).map_err(|e| anyhow::anyhow!(e))?;
