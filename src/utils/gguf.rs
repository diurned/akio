...
impl GgufMeta {
    ...
    pub fn block_count(&self) -> u64 {
        self.get_kv("block_count").and_then(|v| v.as_u64()).unwrap_or(0)
    }

    pub fn embedding_length(&self) -> u64 {
        self.get_kv("embedding_length").and_then(|v| v.as_u64()).unwrap_or(0)
    }
    ...
    fn embedding_head_size(&self) -> u64 {
        ...
        if heads > 0 { self.embedding_length() / heads } else { 0 }
    }
...
fn read_u8<R: Read>(r: &mut R) -> Result<u8, String> {
    let mut b = [0u8; 1];
    r.read_exact(&mut b).map_err(|e| format!("read error: {e}"))?;
    Ok(b[0])
}
...
pub fn parse_gguf(path: &str) -> Result<GgufMeta, String> {
    ...
    if magic != GGUF_MAGIC {
        return Err(format!("'{}' is not a GGUF file (magic 0x{:08X})", path, magic));
    }
    ...
    for _ in 0..tensor_count {
        ...
        tensors.push(TensorInfo { name, shape, tensor_type, offset });
    }

    Ok(GgufMeta { version, kv, tensors })
}
